# ROADMAP — khatm-console (frontend-owned WBS tasks)

> KH-0.1.3 and KH-0.6.1 (FE) were originally planned as separate sessions;
> the FS-C0 foundation session bundled scaffold + contract codegen + i18n +
> API client + auth + app shell + Docker into one session instead. Both rows
> below are done as of that session.

| Task          | Description                                                     | Spec             | Status    |
| ------------- | --------------------------------------------------------------- | ---------------- | --------- |
| KH-0.1.3      | JS → TypeScript strict + toolchain + i18n/RTL scaffold          | —                | ☑ (FS-C0) |
