...
describe('SigningKeysPanel — rotate', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('requires typing the current active key id exactly before rotate is armed, then rotates', async () => {
    ...
    await user.click(
      await screen.findByRole('button', { name: i18n.t('dashboard.keys.rotate.cta') }),
    );
    const dialog = screen.getByRole('dialog');
    const confirm = within(dialog).getByRole('button', {
      name: i18n.t('dashboard.keys.rotate.confirm'),
    });
    expect(confirm).toBeDisabled();

    await user.type(within(dialog).getByRole('textbox'), 'wrong-kid');
    expect(confirm).toBeDisabled();
    expect(within(dialog).getByText(i18n.t('dashboard.keys.rotate.mismatch'))).toBeInTheDocument();
...
describe('SigningKeysPanel — retire', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('shows a Retire action only on RETIRING keys and retires without force on the happy path', async () => {
    ...
    const retireButtons = screen.getAllByRole('button', {
      name: i18n.t('dashboard.keys.retireCta'),
    });
    ...
    const dialog = screen.getByRole('dialog');
    await user.click(
      within(dialog).getByRole('button', { name: i18n.t('dashboard.keys.retire.confirm') }),
    );
  ...
  it('explains KH-KEY-0422 inline and requires a second, severe confirm before forcing', async () => {
    ...
    await screen.findByText('khatm-default:key-1');
    await user.click(screen.getByRole('button', { name: i18n.t('dashboard.keys.retireCta') }));
    await user.click(
      within(screen.getByRole('dialog')).getByRole('button', {
        name: i18n.t('dashboard.keys.retire.confirm'),
      }),
    );

    expect(
      await screen.findByText(i18n.t('dashboard.keys.retire.blocked.title')),
    ).toBeInTheDocument();
    expect(
    ...
    await user.click(
      screen.getByRole('button', { name: i18n.t('dashboard.keys.retire.blocked.forceCta') }),
    );
    expect(
      screen.getByText(i18n.t('dashboard.keys.retire.force.title'), { exact: false }),
    ).toBeInTheDocument();

    await user.click(
      screen.getByRole('button', { name: i18n.t('dashboard.keys.retire.force.confirm') }),
    );
